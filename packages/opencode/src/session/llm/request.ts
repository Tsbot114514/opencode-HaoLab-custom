...
export const prepare = Effect.fn("LLMRequestPrep.prepare")(function* (input: PrepareInput) {
  ...
  const dir = path.join(Global.Path.data, "session", input.sessionID, "system")
  ...
  const replace = (item: SystemSection) =>
    item.id === "structured-output" ? (replacements.structured ?? item.content) : (replacements[item.id] ?? item.content)
  const system = [
