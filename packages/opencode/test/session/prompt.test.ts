...
noLLMServer.instance(
  "loads session-local tools from the session tool directory",
  () =>
    Effect.gen(function* () {
      const sessions = yield* Session.Service
      const provider = yield* ProviderSvc.Service
      const chat = yield* sessions.create({ title: "Session tools" })
      ...
      yield* writeText(
        path.join(Session.folder(chat.id), "tool", "session_echo.ts"),
        `export default {
      ...
      expect(tools.session_echo).toBeDefined()
      const execute = tools.session_echo.execute
      expect(execute).toBeDefined()
      const result = yield* Effect.promise(() =>
        execute!(
          { message: "hello" },
          { toolCallId: "call_session_echo", abortSignal: new AbortController().signal, messages: [] },
        ),
