...
describe("Project.fromDirectory", () => {
  ...
  it.live("uses data path as global worktree for opencode data directory", () =>
    Effect.gen(function* () {
      const { project } = yield* run((svc) => svc.fromDirectory(Global.Path.data))
      expect(project.id).toBe(ProjectID.global)
      expect(project.worktree).toBe(Global.Path.data)
    }),
  )

  it.live("keeps data path global worktree when opening another non-git directory", () =>
    Effect.gen(function* () {
      const tmp = yield* tmpdirScoped()
      yield* run((svc) => svc.fromDirectory(Global.Path.data))
      const { project } = yield* run((svc) => svc.fromDirectory(tmp))
      expect(project.id).toBe(ProjectID.global)
      expect(project.worktree).toBe(Global.Path.data)
    }),
