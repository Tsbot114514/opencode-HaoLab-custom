...
export function SessionRetry(props: { status: SessionStatus; show?: boolean }) {
  ...
  const info = createMemo(() => {
    ...
    const attempt = `${current.attempt}/${current.maxAttempts}`
    if (!line) return i18n.t("ui.sessionTurn.retry.attempt", { attempt })
    return i18n.t("ui.sessionTurn.retry.attemptLine", { line, attempt })
  })
